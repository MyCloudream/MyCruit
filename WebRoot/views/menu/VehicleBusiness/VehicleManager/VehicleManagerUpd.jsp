...
<script type="text/javascript">
	...
	var vids;
	function findVehicleVpartsIds(id){
		$.ajax({
			type : "POST",
			async : false,
			url : "<%=path%>/busi/VehicleManagerUpd.do?method=findVehicleVpartsIds",
			dataType : "json",
			data:{id:id},
			success : function(json) {
				if(json.success){
	...
	$(function() {
        findVehicleVpartsIds(formdata[0].id);
	 ...
	 if(formdata[0].pic1!=null&&formdata[0].pic1.length>1){
		 $("#img1").attr("src", '<%=path%>/common/downloadimg.do?filename=' + formdata[0].pic1 + '&path=img\\vehicle\\'+formdata[0].pic1.substring(0,formdata[0].pic1.indexOf("_"))+'\\' );
		 $("#pic1").val(formdata[0].pic1);
	 } 
	 if(formdata[0].pic2!=null&&formdata[0].pic2.length>1){
		 $("#img2").attr("src", '<%=path%>/common/downloadimg.do?filename=' + formdata[0].pic2 + '&path=img\\vehicle\\'+formdata[0].pic2.substring(0,formdata[0].pic2.indexOf("_"))+'\\' );
		 $("#pic2").val(formdata[0].pic2);
	 } 
	 if(formdata[0].pic3!=null&&formdata[0].pic3.length>1){
		 $("#img3").attr("src", '<%=path%>/common/downloadimg.do?filename=' + formdata[0].pic3 + '&path=img\\vehicle\\'+formdata[0].pic3.substring(0,formdata[0].pic3.indexOf("_"))+'\\' );
		 $("#pic3").val(formdata[0].pic3);
	 } 
	 if(formdata[0].pic4!=null&&formdata[0].pic4.length>1){
		 $("#img4").attr("src", '<%=path%>/common/downloadimg.do?filename=' + formdata[0].pic4 + '&path=img\\vehicle\\'+formdata[0].pic4.substring(0,formdata[0].pic4.indexOf("_"))+'\\' );
		 $("#pic4").val(formdata[0].pic4);
	 } 
	 if(formdata[0].pic5!=null&&formdata[0].pic5.length>1){
		 $("#img5").attr("src", '<%=path%>/common/downloadimg.do?filename=' + formdata[0].pic5 + '&path=img\\vehicle\\'+formdata[0].pic5.substring(0,formdata[0].pic5.indexOf("_"))+'\\' );
		 $("#pic5").val(formdata[0].pic5);
	 } 
	 if(formdata[0].pic6!=null&&formdata[0].pic6.length>1){
		 $("#img6").attr("src", '<%=path%>/common/downloadimg.do?filename=' + formdata[0].pic6 + '&path=img\\vehicle\\'+formdata[0].pic6.substring(0,formdata[0].pic6.indexOf("_"))+'\\' );
			...
			onLoadSuccess:function(data){//当表格成功加载时执行   
              ...
              $.each(rowData,function(idx,val){//遍历JSON;
            	  for (var i = 0; i < vids.length; i++) {
            		  if(val.ncode==vids[i]){
    					datagrid.datagrid("selectRecord", val.id);
    				}
				}
              });   
              initncodevalue();
...
<body class="easyui-layout">
	<div data-options="region:'center',title:'信息录入'">
		<form class="formobj" method="post" action="<%=path%>/busi/VehicleManagerUpd.do">
			<table class="vtbv">
				<tr>
					<th>品牌:</th>
			...
			<div class="imgdiv">
	<!-- 			<input type="text" id="basecodes" name="basecodes" style="width: 500px;"> <input type="text" id="ncodes" name="ncodes"style="width: 500px;"> -->
				<ul>
					<li>
						<p class="pimg">
			...
			<span class="thidden"><input type="hidden" name="subdate"><input type="hidden" name="ccode"><input type="hidden" name="pcode"><input type="hidden" name="uid1"><input type="hidden" name="uid2"><input type="hidden" name="status"><input type="hidden" name="id"><input type="button" id="btn_sub"></span>
		</form>
	</div>
	<div data-options="region:'east',title:'不包涵零配件的选择',collapsible:false" style="width:300px;">
		<table id="tt">
		</table>
	</div>
